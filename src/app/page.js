...
// /**
//  * IoT Distributed Database Dashboard
//  * 
...
// // import NodeHealthCards from '@/components/dashboard/NodeHealthCards';
...
//        const [healthRes, devicesRes, sensorsRes] = await Promise.all([
//          fetch('/api/health'),
//          fetch(`/api/devices?${params}`),
//          fetch(`/api/sensors?${params}&limit=100`),
//        ]);
...
//                  IoT Distributed Database
//                </h1>
//                <p className="text-purple-300 mt-1">
//                  Real-time monitoring & analytics across 4 distributed nodes
//                </p>
//              </div>
...
//              <span>🏥</span> Node Health Status
//            </h2>
//            {healthData && <NodeHealthCards data={healthData} />}
//          </section>
...
//            </h2>
//            {devicesData && sensorsData && (
//              <KPICards devicesData={devicesData} sensorsData={sensorsData} />
//            )}
//          </section>
...
//          <section className="mb-8">
//            <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-2">
//              <span>📈</span> Sensor Analytics
//            </h2>
//            {sensorsData && <SensorCharts data={sensorsData} />}
//          </section>
...
//            </h2>
//            {devicesData && <DeviceTable data={devicesData} />}
//          </section>
...
//              <p className="mb-2">
//                <strong>Distributed Database Concepts Demonstrated:</strong>
//              </p>
...
//                Final Year Project - Distributed Database System © 2025
//              </p>
//            </div>
//          </footer>
//        </main>
...
import AlertsTable from '@/components/dashboard/AlertsTable';
 import FilterBar from '@/components/dashboard/FilterBar';
import TestsPanel from '@/components/dashboard/TestsPanel';
import AlertFilters from '@/components/dashboard/AlertFilters';
// import ReplicationView from '@/components/dashboard/ReplicationView';  
...
export default function Dashboard() {
  ...
  const tabs = [
    ...
    { id: 'tests', name: 'DDB Tests', icon: '⚡' },
    // { id: 'replication', name: 'Replication', icon: '🔄' },
  ];
  ...
  const fetchData = async () => {
    try {
      // Build query params
       ...
       if (alertFilters.region) params.append('region', alertFilters.region);
       if (alertFilters.type) params.append('type', alertFilters.type);
      const [healthRes, devicesRes, sensorsRes, alertsRes] = await Promise.all([
        fetch('/api/health'),
        fetch(`/api/devices?${params}`),
        fetch(`/api/sensors?${params}&limit=100`),
  ...
  useEffect(() => {
   ...
   const handleFilterChange = (newFilters) => {
    ...
    const onFilterChange = (newFilters) => {
     setAlertFilters({ ...alertFilters, ...newFilters });
   };
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {activeTab === 'overview' && (
          <div className="space-y-8">
            <NodeHealthCards data={healthData} />
            <KPICards />
                     {/* Filters */}
         <section className="mb-8">
           <FilterBar filters={filters} onChange={handleFilterChange} />
         </section>
            <SensorCharts data={sensorsData} />
            <DeviceTable data={devicesData} />
          </div>
        )}

        {activeTab === 'analytics' && (
          <SensorCharts data={sensorsData} expanded={true} />
        )}

        {activeTab === 'alerts' && (
          <>
          <AlertFilters filters={alertFilters} onFilterChange={onFilterChange}/>
        ...
        {/* {activeTab === 'replication' && (
          <ReplicationView data={healthData} />
        )} */}
      </main>
    </div>
  );
